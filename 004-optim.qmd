{{< include _setup.qmd >}}
# Model-Free Preference Optimization {#4optim}
...
:::: algorithm
::: algorithmic
**input:** $T$, $B=\{b_1, \dots, b_k\}$ $\delta \gets 1/(TK^2)$ Choose
$\hat{b} \in B$ randomly
...
:::: algorithm
::: algorithmic
**input:** $\gamma$, $\delta$, $w_1$ sample unit vector $u_t$ uniformly
$w_t' \gets P_W(w_t + \delta u_t)$ compare $w_t$ and $w_t'$
$w_{t+1} \gets P_W(w_t + \gamma u_t)$ $w_{t+1} \gets w_t$
:::
::::
...
optimal. Since $\max _{x \in \mathbb{X}} \mathbf{E}_n[f(x)]$ does not
...
3.  $\argmax_{x\in\X}\E^{(n)}[f(x)]=\{2\}$.

4.  $\argmax_{X\in\X^2}\qei^{(n)}(X) = \{(3, 4)\}$.
...
given $\D^{(0)}$ remains the same as the prior; i.e., $p_i^{(0)} = p_i$
for $i=1,2,3,4$. Using this, statements 1 and 2 can be easily verified.
Now note that $\E^{(0)}[f(1)]=-1$, $\E^{(0)}[f(2)]=0$, and
$\E^{(0)}[f(3)] = \E^{(0)}[f(4)] = \frac{3}{2}(p - q)$. Since $p < q$,
it follows that $\argmax_{x\in\X}\E^{(n)}[f(x)]=\{2\}$; i.e., statement
3 holds. Finally, since $\max_{x\in\{1,2\}}\E^{(0)}[f(x)] = 0$, the qEI
...
Since $\max_{x\in\X}\E^{(0)}[f(x)] = 0$, the qEI acquisition function at
time $n+1$ is given by
$\qei^{(n+1)}(X) = \E^{(n+1)}[\{\max\{f(x_1), f(x_2)\}\}^+]$. Since
$f(1) \leq f(x)$ almost surely under the prior for all $x\in\X$, there
...
$\argmax_{X \in X^2}\text{qEI}^{(n+1)}(X) = \{(3, 4)\}$, which concludes
the proof by induction.

Finally, since $\argmax_{x\in X}\E^{(n)}[f(x)]=\{2\}$ for all $n$, the
