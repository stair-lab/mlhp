...
<iframe
  src="../slides/4.1.dueling_bandit.html"
  style="width:45%; height:225px;"
></iframe>
<iframe
  src="../slides/4.2.preferential_baysian_optimization.html"
  style="width:45%; height:225px;"
></iframe>
[Fullscreen Part 1](../slides/4.1.dueling_bandit.html){.btn .btn-outline-primary .btn role="button"}
[Fullscreen Part 2](../slides/4.2.preferential_baysian_optimization.html){.btn .btn-outline-primary .btn role="button"}
...
<!-- TODO: Briefly introduce expected improvement as another acquisition function -->
...
and aims to find a Von Neumann winner by minimizing cumulative average regret:
...
**Recommendation systems** are essential components of numerous online platforms, guiding users
...
\end{aligned}$$ {#eq-claim1-proof} as claimed.

**Claim 2** $\mathrm{qEUBO}_n(X) \leq V_n(X) .$ For any given
$X \in \mathbb{X}^q$ we have
$$\mathbb{E}_n\left[f\left(x_{r(X)}\right) \mid(X, r(X))\right] \leq \max _{x \in \mathbb{X}} \mathbb{E}_n[f(x) \mid(X, r(X))] .$$ {#eq-claim2-inequality}
...
\end{aligned}$$ {#eq-contradiction-chain}
...
f_4(x) = \begin{cases}
    ...
    -\frac{1}{2}, &\ x=4
    \end{cases}.
\end{aligned}$$ {#eq-qei-counterexample-functions}
...
p_i = Pr(f=f_i) =
    \begin{cases}
        ...
        q/2, i=3,4.
    \end{cases}
\end{aligned}$$ {#eq-qei-prior} We also assume the user's response likelihood is given
...
p_i^{(n+1)} \propto \begin{cases}
                        p_i^{(n)}\ell, \ i=1,3,\\
                         p_i^{(n)} (1 - \ell), \ i=2,4,
                        \end{cases}
\end{aligned}$$ {#eq-qei-posterior-update} where
$$\ell = a I\{r^{(n+1)} = 1\} + (1-a)I\{r^{(n+1)} = 2\}.$$ {#eq-ell-definition} Observe that
...
Now observe that $$\begin{aligned}
    ...
    &\leq 0,
\end{aligned}$$ {#eq-expected-f3} where the last inequality holds since
...
$$\text{qEI}^{(n+1)}(2, 3) = p_1^{(n+1)} + 1/2 p_2^{(n+1)},$$ {#eq-qei-23}
$$\operatorname{qEI}^{(n+1)}(3, 4) = p_1^{(n+1)} + p_2^{(n+1)}$$ {#eq-qei-34} and
$$\operatorname{qEI}^{(n+1)}(4, 2) = 1/2p_1^{(n+1)} + p_2^{(n+1)}.$$ {#eq-qei-42}
...
Bayesian simple regret of qEI is given by $$\begin{aligned}
    ...
    &= p
\end{aligned}$$ {#eq-qei-final-regret} for all $n$. ◻
:::
...
1.  Linear: $$k(x, \bar{x})=x^{\top} \bar{x} .$$ {#eq-kernel-linear}

2.  Squared Exponential (SE):
    $$k(x, \bar{x})=\sigma_{\mathrm{SE}}^2 \exp \left\{-\frac{\|x-\bar{x}\|^2}{l^2}\right\},$$ {#eq-kernel-se}
    where $\sigma_{\mathrm{SE}}^2$ is the variance parameter and $l$ is
    the lengthscale parameter.

3.  Matérn:
    $$k(x, \bar{x})=\frac{2^{1-\nu}}{\Gamma(\nu)}\left(\sqrt{2 \nu} \frac{\|x-\bar{x}\|}{\rho}\right)^\nu K_\nu\left(\sqrt{2 \nu} \frac{\|x-\bar{x}\|}{\rho}\right),$$ {#eq-kernel-matern}
...
where
$$\beta_T=\beta(1 / T, \delta, T)=\mathcal{O}\left(\sqrt{T \log \frac{T \mathcal{N}\left(\mathcal{B}_f, 1 / T,\|\cdot\|_{\infty}\right)}{\delta}}\right).$$ {#eq-beta-t}
:::
...
1.  If $k(x, y)=\langle x, y\rangle$, we have,
    $$R_T=\mathcal{O}\left(T^{3 / 4}(\log T)^{3 / 4}\right) .$$ {#eq-regret-linear-kernel}

2.  If $k(x, y)$ is a squared exponential kernel, we have,
    $$R_T=\mathcal{O}\left(T^{3 / 4}(\log T)^{3 / 4(d+1)}\right) .$$ {#eq-regret-se-kernel}

3.  If $k(x, y)$ is a Matérn kernel, we have,
    $$\left.R_T=\mathcal{O}\left(T^{3 / 4}(\log T)^{3 / 4} T^{\frac{d}{\nu}\left(\frac{1}{4}+\frac{d+1}{4+2(d+1)^d / \nu}\right.}\right)\right).$$ {#eq-regret-matern-kernel}
:::
...
## Summary of Notation {#sec-notation-summary-chap5}

| Symbol | Description |
|--------|-------------|
| $U_i \in \mathbb{R}^K$ | User latent embedding |
| $V_j \in \mathbb{R}^K$ | Item feature vector |
| $H_{ij} = h_i(V_j)$ | Latent utility of item $j$ for user $i$ |
| $\mathcal{D}_t$ | Dataset of observations up to time $t$ |
| $\tilde{U}_i^{(t)}$ | Thompson Sampling posterior sample at time $t$ |
| $\sigma(\cdot)$ | Logistic sigmoid function |
| $\mu_a = \mathbb{E}[r_t \mid a]$ | Expected reward of arm $a$ |
...
| $P_{ij} = \Pr(i \succ j)$ | Preference probability of arm $i$ over $j$ |
| $\pi_f([\mathbf{x}, \mathbf{x}'])$ | Preference function for duel |
| $S(\mathbf{x})$ | Copeland score |
| $C(\mathbf{x})$ | Soft-Copeland score |
| $\mathbf{x}_c$ | Condorcet winner |
| $g(\cdot)$ | Latent objective function in PBO |
| $\mathcal{B}_g$ | Confidence set for $g$ |
| $\text{qEUBO}_n(X)$ | Expected utility of best option acquisition function |
| $K_t$ | GP kernel matrix at time $t$ |
| $W$ | Diagonal matrix of $p(1-p)$ terms in Laplace approximation |
