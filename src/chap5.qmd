...
1. **Social Choice Theory** (@sec-aggregation–@sec-arrow): We introduce the formal framework of social choice, present Arrow's and Gibbard–Satterthwaite's impossibility theorems, and discuss their implications for AI systems that aggregate human feedback.

2. **Escaping Impossibility** (@sec-domain-restrictions–@sec-borda-dpo): We show how restricting the domain of preferences (e.g., to single-peaked preferences) enables strategy-proof aggregation, and we prove a key connection between the Borda count and DPO.
