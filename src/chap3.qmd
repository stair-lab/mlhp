...
We denote $Y_{ij}$ to be the response of user $i$ to item $j$, which is drawn from a collection of $M$ items. The response is $1$ if the user likes the item and zero otherwise. The Rasch model posits that response is a Bernoulli random variable governed by user's appetite $U_i \in \mathbb{R}$ and item's appeal $V_j \in \mathbb{R}$: 

$$
p(Y_{ij} = 1 | U_i, V_j) = \sigma(H_{ij}), \quad H_{ij} = U_i + V_j
$$
...
$$
p(Y_{ij} = 1 | U_i, V_j, Z_j) = \sigma(H_{ij}), \quad H_{ij} = d(U_i, V_j + Z_j),
$$
