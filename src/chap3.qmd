...
Before introducing models for preference data, it is useful to begin with some stylized empirical observations. For example, surveys indicate that only a small fraction of the US population reports liking durian, while a much larger fraction endorses liking banana or strawberry. Some people appear to have a general fondness for fruit, while others are relatively indifferent or even averse. Interestingly, even within fruit-likers, there is heterogeneity: some prefer durian over banana, while others show the opposite ordering. These facts suggest several latent forces at work (appetites, baseline appeal, trait heterogeneity) that any plausible data-generating process (DGP) for preference must accommodate. Below we propose one such DGP as a conceptual scaffold for understanding the observed patterns.

We denote the response of user $i$ to item $j$ by $Y_{i,j}$, which is drawn from a collection of $M$ items. The response is $1$ if the user likes the item and $0$ otherwise. The Rasch model posits that the response is a Bernoulli random variable governed by user's appetite $U_i \in \mathbb{R}$ and item's appeal $V_j \in \mathbb{R}$: 

$$
p(Y_{i,j} = 1 | U_i, V_j) = \sigma(H_{i,j}), \quad H_{ij} = U_i + V_j
$$
where $\sigma$ is the logistic function. The user appetite is a latent variable that represents the appetite construct, which is the tendency of the user to like items in the collection in general, and the item appeal is how easy it is for the item to be liked by the population of users.

The user can express their affinity via several mechanisms. For example: thumbs-up/down on an e-commerce site, swiping on a dating site, or skipping a track on a music app. User- and item-specific parameters do not need to be unidimensional. When appetite and appeal are $K$-dimensional, the data generating process is
$$
p(Y_{i,j} = 1 | U_i, V_j, Z_j) = \sigma(H_{i,j}), \quad H_{ij} = d(U_i, V_j) + Z_j,
$$
where $d$ is some distance function (not strictly a distance metric) in the $K$-dimensional space, $U_i$ is the user-specific appetite for each dimension, $V_j$ is the item loading, and $Z_j$ is the offset. When $d$ is the negative Euclidean distance, the model is known as the Ideal Point Model, which is popular in political sciences and psychometrics:
$$
H_{ij} = -||U_i -  V_j ||_2 + Z_j
$$
In this case, $U_i$ is the user embedding and $V_j$ is the item embedding with some scalar offset $Z_j$. When the score/similarity is the dot product, the above generating process is known as the logistic factor model, a popular class of model in psychometrics:
$$
H_{ij} = U_i^\top V_j + Z_j
$$

Each of the $K$ dimensions represents different properties of the items. For example, when $K=2$, a fruit might be sweet and have a strong aroma, and some users might look for sweetness, while others who might avoid sugar and look for strong aroma.

In many cases, the user might be asked to express their preference in terms of comparison between two items $j$ and $j'$, in which the response $Y_{i,jj'}$ is $1$ if the item $j$ is preferred over $j'$ and $0$ otherwise. 
$$
p(Y_{i,jj'} = 1 | i, j, j') = \sigma(H_{i,j} - H_{i,j'}) = \frac{\exp(H_{ij})}{\exp(H_{ij}) + \exp(H_{ij'})},
$$
where the last equality uses the logistic identity. This is also known in the literature as the Bradley-Terry model. For a single user, if the generating process for item-wise preference is Rasch, we can further simplify $H_{i,j} - H_{i,j'} = U_i + V_j - U_i - V_{j'} = V_j - V_{j'}$. Hence, for a single user, the pairwise logit does not depend on the user-specific appetite, but only on the item parameters. This cancellation happens only for single-user pairwise comparisons and is generally not true for other classes of model.

In the item-wise preference, the responses for $N$ users and $M$ items are a 2D matrix with shape $N \times M$, while in the pairwise preference, the responses for the same number of users and items are a 3D tensor with shape $N \times M \times M$. 

Given a data generating process, we can synthesize some data. The code below generates a Rasch response matrix: 
...
Assuming a single user, and that the item-wise generating process is Rasch, we generate their Bradley–Terry response below.
