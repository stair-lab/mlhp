---
title: "Whose Preferences Matter?"
format: html
filters:
  - pyodide
execute:
...
**From Chapter 6 (Aggregation)**: Arrow's theorem and Sen's Impossibility of a Paretian Liberal (@sec-impossibility-results) show that no aggregation mechanism satisfies all desirable properties. In @sec-fairness-impossibility, we'll see how adding fairness constraints further limits our options.
...
- **@sec-governance** examines governance mechanisms for deciding whose preferences matter: designer discretion, user control, participatory design, regulatory oversight, and professional standards.
...
This isn't a technical question—it's a normative question about when paternalism is justified. We draw on Amartya Sen's framework from Chapter 6 (@sec-sen-liberal).

#### Sen's Framework: Nosy vs. Non-Nosy Preferences {#sec-nosy-preferences}

From Chapter 6, recall Sen's "Impossibility of a Paretian Liberal" (@sec-sen-liberal). Sen distinguishes between:
...
**Connection to Chapter 6**: Arrow's theorem (@sec-arrow) and Sen's Paretian Liberal (@sec-sen-liberal) show impossibility results—no mechanism satisfies all desirable properties. Adding fairness constraints makes this worse. We must choose which properties to prioritize.
...
**Principle**: Accept that individual/group and process/outcome fairness are often incompatible (@sec-fairness-impossibility). Choose based on domain and stakes. Be transparent about choice and justify.
...
**Connection**: Governance question from @sec-governance (to be written in Phase 3). Complements Principle 1 (make tradeoffs explicit) by involving stakeholders in the tradeoff decisions.
