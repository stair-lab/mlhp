...
could also be employed to select data for fine-tuning large language models (LLMs) for specific
...
role of LLMs in assisting robots through corrective feedback and highlight 
...
2. **$\theta_1,...,\theta_n$**: The objects, now representing their locations in $R^d$.
...
$$\begin{aligned}
\phi(C) &= a_{11} T P + a_{01} F P + a_{10} F N + a_{00} TN\\
&= m_{11} T P + m_{00} T N + m_{0},
\end{aligned}$$ 
...
$$\begin{aligned}
\phi(C) &= \frac{a_{11} T P + a_{01} F P + a_{10} F N + a_{00} T N}{b_{11} T P + b_{01} F P + b_{10} F N + b_{00} T N}\\
&= \frac{p_{11} T P + p_{00} T N + p_{0}}{q_{11} T P + q_{00} T N + q_{0}}
\end{aligned}$$ 
...
::: {.content-visible when-format="html"}
$\Gamma\left(h, h^{\prime}\right)=\Omega\left(C, C^{\prime}\right)=\unicode{x1D7D9}\left[\phi(C)>\phi\left(C^{\prime}\right)\right]=: \unicode{x1D7D9} \left[C \succ C^{\prime}\right],$
:::
...
#| label: alg-lpm
\begin{algorithm}
    \caption{Quasiconcave Metric Maximization}\label{lpm}
    \begin{algorithmic}
...
the elicited metric $\hat{\phi}$.

The pseudocode for LFPM elicitation is given in @alg-lfpm.
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \For{$p'_{11} = 0; \; p'_{11} \leq 1; \; p'_{11} = p'_{11} + \Delta$}
            \State Compute $\phi'$, $\phi''$ using Proposition 4. Compute array $r = \left[ \frac{\phi'(C_1)}{\phi''(C_1)}, \dots, \frac{\phi'(C_k)}{\phi''(C_k)} \right]$. Set $\sigma = \text{std}(r)$.
...
satisfying our assumptions, @alg-lpm or @alg-lfpm finds an approximate
...
These two theorems ensure that @alg-lpm and @alg-lfpm find an appropriate
...
In this section, we have introduced the framework of metric elicitation
